...
import { AlertTriangle, Bell, CalendarDays, CheckCircle2, ClipboardList, CloudOff, Copy, FileText, History, LayoutDashboard, Mail, PauseCircle, Pencil, PhoneCall, Plus, RefreshCw, RotateCcw, Settings2, Sprout, Trash2, UserRoundPlus, Users, XCircle } from 'lucide-react';
...
export function AdminPage({ repository, navigate, revision, onChanged }: { repository: AdminRepository; navigate: Navigate; revision: number; onChanged: () => void }) {
  ...
  const [participantTarget, setParticipantTarget] = useState<CalendarSlot | null>(null);
  ...
  return (
    <div className="admin-shell">
      ...
      <div className="admin-content">
        ...
        {tab === 'slots' && <SlotManagement slots={slots} experiences={experiences} bookings={bookings} waitlist={waitlist} repository={repository} run={run} busy={busy} onParticipants={setParticipantTarget} onCancel={(slot) => setCancelTarget(slot)} onSaved={async () => { onChanged(); await load(); }} onViewPublic={() => navigate('/')} />}
      ...
      {cancelTarget !== undefined && <CancelSlotModal initialSlot={cancelTarget} slots={slots.filter((slot) => slot.publicationStatus === 'published' && slot.manualStatus !== 'cancelled')} bookings={bookings} waitlist={waitlist} onClose={() => setCancelTarget(undefined)} onConfirm={async (slot, reason, expectedTargetIds) => { const succeeded = await run(`cancel-${slot.id}`, () => repository.cancelSlot(slot.id, reason, expectedTargetIds)); if (succeeded) setCancelTarget(undefined); }} busy={busy.startsWith('cancel-')} />}
      {participantTarget && <ParticipantListModal slot={participantTarget} bookings={bookings} waitlist={waitlist} onClose={() => setParticipantTarget(null)} onCancel={() => { setParticipantTarget(null); setCancelTarget(participantTarget); }} />}
    </div>
  );
}
...
function SlotManagement({ slots, experiences, bookings, waitlist, repository, run, busy, onParticipants, onCancel, onSaved, onViewPublic }: { slots: CalendarSlot[]; experiences: Experience[]; bookings: Booking[]; waitlist: WaitlistEntry[]; repository: AdminRepository; run: (key: string, task: () => Promise<unknown>) => void; busy: string; onParticipants: (slot: CalendarSlot) => void; onCancel: (slot: CalendarSlot) => void; onSaved: () => Promise<void>; onViewPublic: () => void }) {
...
function CancelSlotModal({ initialSlot, slots, bookings, waitlist, onClose, onConfirm, busy }: { initialSlot: CalendarSlot | null; slots: CalendarSlot[]; bookings: Booking[]; waitlist: WaitlistEntry[]; onClose: () => void; onConfirm: (slot: CalendarSlot, reason: string, expectedTargetIds: string[]) => Promise<void>; busy: boolean }) {
  const [slotId, setSlotId] = useState(initialSlot?.id ?? slots[0]?.id ?? '');
  const [reason, setReason] = useState('生育・天候状況により開催を中止します。');
  const [reviewedTargetIds, setReviewedTargetIds] = useState<string[] | null>(null);
  ...
  const affectedCount = affectedBookings.length + affectedWaitlist.length;
  const expectedTargetIds = [...affectedBookings.map((booking) => booking.id), ...affectedWaitlist.map((entry) => entry.id)];
  const targetsStillMatch = reviewedTargetIds !== null && reviewedTargetIds.length === expectedTargetIds.length && reviewedTargetIds.every((targetId, index) => targetId === expectedTargetIds[index]);
  const notificationCopy = slot ? buildNotificationCopy('slotCanceled', { experience: slot.experience.name, date: format(new Date(slot.startAt), 'M月d日(E) H:mm', { locale: ja }), reason: reason.trim() }) : null;
  return <Modal title="開催中止と連絡の確認" onClose={onClose} className="cancel-slot-modal"><form className="cancel-slot-form" onSubmit={async (event) => { event.preventDefault(); if (slot && reason.trim() && targetsStillMatch && reviewedTargetIds) await onConfirm(slot, reason.trim(), reviewedTargetIds); }}><div className="cancel-flow" aria-label="開催中止の手順"><span className="is-complete">1 対象者確認</span><span className="is-current">2 通知内容確認</span><span>3 中止確定</span></div><div className="cancel-slot-alert"><CloudOff /><div><strong>中止確定と同時に対象者へ通知します</strong><p>販売デモでは実際のメールは送らず、対象者ごとの通知履歴と操作履歴を作成します。</p></div></div><label><span>対象の開催枠</span><select value={slotId} onChange={(event) => { setSlotId(event.target.value); setReviewedTargetIds(null); }} required>{slots.map((item) => <option value={item.id} key={item.id}>{format(parseISO(item.startAt), 'M/d（E）H:mm', { locale: ja })} {item.experience.name}</option>)}</select></label>{slot && <div className="cancel-impact"><div><span>体験</span><strong>{slot.experience.name}</strong></div><div><span>日時</span><strong>{format(parseISO(slot.startAt), 'M月d日（E）H:mm', { locale: ja })}</strong></div><div><span>予約者</span><strong>{affectedBookings.reduce((sum, booking) => sum + booking.totalPeople, 0)}名・{affectedBookings.length}組</strong></div><div><span>待機者</span><strong>{affectedWaitlist.reduce((sum, entry) => sum + entry.totalPeople, 0)}名・{affectedWaitlist.length}組</strong></div></div>}<AffectedParticipantList bookings={affectedBookings} waitlist={affectedWaitlist} /><label><span>中止理由 <small>必須</small></span><textarea value={reason} maxLength={200} rows={3} onChange={(event) => { setReason(event.target.value); setReviewedTargetIds(null); }} required /></label>{notificationCopy && <section className="cancel-notification-preview"><header><Mail /><div><span>送信される通知</span><strong>{notificationCopy.subject}</strong></div></header><p>{notificationCopy.preview}</p><small>対象者{affectedCount}組それぞれの通知履歴に同じ内容を記録します。</small></section>}<label className="cancel-review-check"><input type="checkbox" checked={targetsStillMatch} onChange={(event) => setReviewedTargetIds(event.target.checked ? [...expectedTargetIds] : null)} /><span><strong>対象者と通知内容を確認しました</strong><small>確定後は予約・待機状態が「開催中止」に変わります</small></span></label><div className="modal-actions"><Button variant="secondary" type="button" onClick={onClose}>中止しない</Button><Button variant="danger" type="submit" disabled={!slot || !reason.trim() || !targetsStillMatch || busy}>{busy ? '通知履歴を作成中…' : `対象${affectedCount}組へ通知して中止`}</Button></div></form></Modal>;
}

function AffectedParticipantList({ bookings, waitlist }: { bookings: Booking[]; waitlist: WaitlistEntry[] }) {
  const people = [
    ...bookings.map((booking) => ({ id: booking.id, kind: '予約確定', name: booking.contact.name, phone: booking.contact.phone, email: booking.contact.email, total: booking.totalPeople })),
    ...waitlist.map((entry) => ({ id: entry.id, kind: `待機 ${entry.queueNumber}番`, name: entry.contact.name, phone: entry.contact.phone, email: entry.contact.email, total: entry.totalPeople })),
  ];
  return <section className="affected-participants" aria-label="連絡対象者"><header><div><Users /><strong>連絡対象者</strong></div><span>{people.length}組</span></header>{people.length ? <ul>{people.map((person) => <li key={person.id}><span className="participant-kind">{person.kind}</span><div><strong>{person.name}<small>{person.total}名</small></strong><p><PhoneCall />{person.phone}</p><p><Mail />{person.email}</p></div></li>)}</ul> : <p className="participant-empty">予約者・待機者はいません。通知なしで開催枠のみ中止します。</p>}</section>;
}

function ParticipantListModal({ slot, bookings, waitlist, onClose, onCancel }: { slot: CalendarSlot; bookings: Booking[]; waitlist: WaitlistEntry[]; onClose: () => void; onCancel: () => void }) {
  const affectedBookings = bookings.filter((booking) => booking.slotId === slot.id && booking.status === 'confirmed');
  const affectedWaitlist = waitlist.filter((entry) => entry.slotId === slot.id && entry.status === 'waiting');
  return <Modal title="参加者・連絡先" onClose={onClose} className="participant-list-modal"><div className="participant-list-dialog"><div className="participant-slot-summary"><span>{format(parseISO(slot.startAt), 'M月d日（E）H:mm', { locale: ja })}</span><strong>{slot.experience.name}</strong><small>予約 {affectedBookings.reduce((sum, booking) => sum + booking.totalPeople, 0)}名 ／ 待機 {affectedWaitlist.reduce((sum, entry) => sum + entry.totalPeople, 0)}名</small></div><AffectedParticipantList bookings={affectedBookings} waitlist={affectedWaitlist} /><p className="participant-demo-note">販売デモのため、氏名・連絡先は固定ダミー値です。</p><div className="modal-actions"><Button variant="secondary" onClick={onClose}>閉じる</Button>{slot.manualStatus !== 'cancelled' && <Button variant="danger" onClick={onCancel}><CloudOff />この枠を中止する</Button>}</div></div></Modal>;
}
