...
test('電話予約も入力した代表者情報を固定ダミー値へ置換する', async () => {
  const repository = new DemoRepository();
  const slots = await repository.listSlots();
  const booking = await repository.createPhoneBooking({
    ...
    contact: {
      name: '電話予約デモ',
      email: 'phone-demo@example.invalid',
      phone: '000-1111-2222',
      note: '折り返し連絡を希望',
    },
  ...
  assert.deepEqual(booking.contact, {
    name: 'デモ利用者',
    email: 'demo@example.invalid',
    phone: '000-0000-0000',
    note: 'デモ入力のため保存していません',
  });
  assert.equal((await repository.lookupBooking(booking.code, 'demo@example.invalid'))?.id, booking.id);
  const notification = (await repository.listNotificationJobs()).find((job) => job.targetId === booking.id);
  assert.equal(notification?.recipientName, 'デモ利用者');
  assert.equal(notification?.recipientEmail, 'demo@example.invalid');
  assert.doesNotMatch(storage.dump(), /電話予約デモ|phone-demo@example\.invalid|000-1111-2222|折り返し連絡を希望/);
  await assert.rejects(repository.createPhoneBooking({
    slotId: slots[0].id,
    party: { adults: 1, children: 0, infants: 0 },
...
test('電話予約の人数内訳を保存し、合計人数分だけ残席を減らす', async () => {
  const repository = new DemoRepository();
  const slot = await repository.createSlot({ ...slotInput(90), capacity: 10, fewThreshold: 2 });
  const before = (await repository.listSlots()).find((item) => item.id === slot.id);
  assert.equal(before?.remaining, 10);

  const booking = await repository.createPhoneBooking({
    slotId: slot.id,
    party: { adults: 2, children: 1, infants: 1 },
    contact: { name: '電話予約', email: '', phone: '090-0000-0000' },
    sendNotification: false,
  });

  assert.deepEqual(booking.party, { adults: 2, children: 1, infants: 1 });
  assert.equal(booking.totalPeople, 4);
  const after = (await repository.listSlots()).find((item) => item.id === slot.id);
  assert.equal(after?.bookedPeople, 4);
  assert.equal(after?.remaining, 6);
});

test('管理者の個別キャンセルは予約を終端状態へ遷移させ残席と監査ログを更新する', async () => {
  const repository = new DemoRepository();
  const slot = await repository.createSlot({ ...slotInput(91), capacity: 6, fewThreshold: 2 });
  const booking = await repository.createPhoneBooking({
    slotId: slot.id,
  ...
  const canceled = await repository.cancelBookingByAdmin(booking.id, 'お客様から電話連絡');
  assert.equal(canceled.status, 'canceledByAdmin');
  const managedSlot = (await repository.listSlots()).find((item) => item.id === slot.id);
  assert.equal(managedSlot?.remaining, 6);
  assert.equal((await repository.listAuditLogs()).some((log) => log.action === 'ADMIN_BOOKING_CANCELED' && log.targetId === booking.id), true);
  await assert.rejects(repository.cancelBookingByAdmin(booking.id, '重複操作'), /有効な予約だけ/);
});

test('受付済みへ遷移しても予約人数と残席を維持し監査ログへ記録する', async () => {
  const repository = new DemoRepository();
  const slot = await repository.createSlot({ ...slotInput(89), capacity: 6, fewThreshold: 2 });
  const booking = await repository.createPhoneBooking({
    slotId: slot.id,
    party: { adults: 2, children: 1, infants: 0 },
    contact: { name: '受付テスト', email: '', phone: '090-0000-0000' },
    sendNotification: false,
  });

  const checkedIn = await repository.markBookingCheckedIn(booking.id);
  assert.equal(checkedIn.status, 'checkedIn');
  const managedSlot = (await repository.listSlots()).find((item) => item.id === slot.id);
  assert.equal(managedSlot?.bookedPeople, 3);
  assert.equal(managedSlot?.remaining, 3);
  assert.equal((await repository.listAuditLogs()).some((log) => log.action === 'BOOKING_CHECKED_IN' && log.targetId === booking.id), true);
  await assert.rejects(repository.markBookingCheckedIn(booking.id), /確定中の予約だけ/);
});
...
test('開催枠作成から公開・予約・管理反映まで一貫して更新される', async () => {
  const repository = new DemoRepository();
  const created = await repository.createSlot({
    ...slotInput(120, 'published'),
    capacity: 6,
    fewThreshold: 2,
    note: '通し検証用',
  });
  const month = format(new Date(created.startAt), 'yyyy-MM');

  const publicSlot = (await repository.listCalendar(month)).find((slot) => slot.id === created.id);
  assert.ok(publicSlot, '公開した開催枠が利用者カレンダーに表示される');
  assert.equal(publicSlot.bookedPeople, 0);
  assert.equal(publicSlot.remaining, 6);

  const booking = await repository.createBooking({
    slotId: created.id,
    party: { adults: 1, children: 1, infants: 0 },
    contact: { name: '通し検証', email: 'test@example.com', phone: '090-0000-0000' },
  });
  assert.equal(booking.status, 'confirmed');
  assert.equal(booking.totalPeople, 2);

  const managedSlot = (await repository.listSlots()).find((slot) => slot.id === created.id);
  assert.ok(managedSlot, '予約後も管理画面の開催枠一覧に存在する');
  assert.equal(managedSlot.bookedPeople, 2);
  assert.equal(managedSlot.remaining, 4);
  assert.equal((await repository.listBookings(created.id)).length, 1);
});
...
test('待機は待機中から予約へ一度だけ繰り上がる', async () => {
  ...
  assert.equal((await repository.listSlots()).find((item) => item.id === slot.id)?.remaining, 0);
  assert.equal((await repository.listAuditLogs()).find((log) => log.targetId === waiting.id)?.action, 'WAITLIST_PROMOTED');
...
test('開催中止は確定予約と待機を終端状態へ遷移させ、その後の申込を拒否する', async () => {
  ...
  const canceledSlot = await repository.cancelSlot(slot.id, '荒天のため中止', [booking.id, waiting.id]);
  ...
  const cancelNotifications = (await repository.listNotificationJobs()).filter((job) => job.type === 'slotCanceled');
  assert.equal(cancelNotifications.length, 2);
  assert.ok(cancelNotifications.every((job) => job.preview.includes('中止理由：荒天のため中止')));
...
test('開催枠の受付停止と再開を即時保存し、専用の操作履歴を残す', async () => {
  const repository = new DemoRepository();
  const slot = await repository.createSlot(slotInput(104));
  const expectedDateTime = format(new Date(slot.startAt), 'M/d H:mm');

  const paused = await repository.setSlotPaused(slot.id, true);
  assert.equal(paused.manualStatus, 'paused');
  assert.equal((await repository.listSlots()).find((item) => item.id === slot.id)?.displayStatus, 'paused');
  const pausedLog = (await repository.listAuditLogs()).find((log) => log.action === 'SLOT_PAUSED');
  assert.equal(pausedLog?.summary, `${expectedDateTime} いちご狩り体験を受付停止にしました`);

  const resumed = await repository.setSlotPaused(slot.id, false);
  assert.equal(resumed.manualStatus, 'normal');
  assert.notEqual((await repository.listSlots()).find((item) => item.id === slot.id)?.displayStatus, 'paused');
  const resumedLog = (await repository.listAuditLogs()).find((log) => log.action === 'SLOT_RESUMED');
  assert.equal(resumedLog?.summary, `${expectedDateTime} いちご狩り体験の受付を再開しました`);
  await assert.rejects(repository.setSlotPaused(slot.id, false), /すでに通常受付中/);
});

test('公開した開催枠を原子的に保存し、日時・体験・定員を操作履歴へ残す', async () => {
  const repository = new DemoRepository();
  const input = { ...slotInput(107), capacity: 20, publicationStatus: 'published' as const };
  const expectedDateTime = format(new Date(input.startAt), 'M/d H:mm');
  const beforeCount = (await repository.listSlots()).length;

  const created = await repository.createSlot(input);
  const stored = (await repository.listSlots()).find((slot) => slot.id === created.id);
  const log = (await repository.listAuditLogs()).find((item) => item.targetId === created.id && item.action === 'SLOT_CREATED');

  assert.equal((await repository.listSlots()).length, beforeCount + 1);
  assert.equal(stored?.publicationStatus, 'published');
  assert.equal(stored?.capacity, 20);
  assert.equal(log?.summary, `${expectedDateTime} いちご狩り体験を定員20名で公開しました`);
});

test('生育調整中と開催中止済みの枠は受付トグルで上書きしない', async () => {
  const repository = new DemoRepository();
  const adjusting = await repository.createSlot({ ...slotInput(105), manualStatus: 'adjusting' });
  const cancelled = await repository.createSlot(slotInput(106));
  await repository.cancelSlot(cancelled.id, '安全確認のため', []);

  await assert.rejects(repository.setSlotPaused(adjusting.id, true), /生育調整中/);
  await assert.rejects(repository.setSlotPaused(cancelled.id, false), /開催中止済み/);
});

test('開催中止の確認後に対象者が変わった場合は全処理を中止して再確認を求める', async () => {
  const repository = new DemoRepository();
  const slot = await repository.createSlot({ ...slotInput(103), capacity: 4 });
  const reviewedBooking = await repository.createBooking({
    slotId: slot.id,
    party: { adults: 1, children: 0, infants: 0 },
    contact: { name: '確認済み', email: 'reviewed@example.invalid', phone: '000-0000-0000' },
  });
  await repository.createBooking({
    slotId: slot.id,
    party: { adults: 1, children: 0, infants: 0 },
    contact: { name: '確認後', email: 'new@example.invalid', phone: '000-0000-0000' },
  });
  const notificationCount = (await repository.listNotificationJobs()).length;

  await assert.rejects(
    repository.cancelSlot(slot.id, '荒天のため中止', [reviewedBooking.id]),
    /対象者をもう一度確認/,
  );

  assert.equal((await repository.listSlots()).find((item) => item.id === slot.id)?.manualStatus, 'normal');
  assert.ok((await repository.listBookings(slot.id)).every((bookingItem) => bookingItem.status === 'confirmed'));
  assert.equal((await repository.listNotificationJobs()).length, notificationCount);
});
...
test('通知は失敗から送信済みへだけ再送でき、二重再送を拒否する', async () => {
  ...
  assert.ok(sent.updatedAt);
  assert.equal((await repository.listAuditLogs()).find((log) => log.targetId === failed.id)?.action, 'RETRY_NOTIFICATION');
